...
Consider that $NL$(\textbf{x}) represents the objective function in \eqref{eq:pnl}, where \textbf{x} stands for the decision variable vector. In our case, $\textbf{x}\in[G, d, R_1, R_2]$. Note that $G$ is included in the vector, though it does not directly affect the $NL$ of $V_{\theta}$. However, inclusion of $G$ enables us to obtain the minimum gain needed to meet the $S_L$ condition, while satisfying other constraints. As a first step for minimizing $NL$, an initial population, with $PS$ number of \textbf{x}, is randomly and uniformly selected within the lower and upper bounds of \textbf{x}.
